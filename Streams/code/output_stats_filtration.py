...
outputs_df = pd.read_csv(os.path.join(output_dir,'streams-filtration_ML_results_compiled.csv'))
...
treatments = outputs_df.treatment.unique()
...
rmse_anova = smf.ols('trans_val ~ C(model) + C(species) + C(treatment) +'+\
                          'C(model):C(treatment) + C(species):C(treatment) + C(model):C(species) +'+\
                              'C(model):C(species):C(treatment)',
                          data = test_rmses).fit()

sm.qqplot(rmse_anova.resid)

rsq_anova = smf.ols('trans_val ~ C(model) + C(species) + C(treatment) +'+\
                          'C(model):C(treatment) + C(species):C(treatment) + C(model):C(species) +'+\
                              'C(model):C(species):C(treatment)',
                          data = test_rsqs).fit()
...
for perf_met in list(perf_mets.keys()):
    ...
    for s in species:
        ...
        for m in models:
            ...
            pm_tukey.plot_simultaneous(ax = ax, xlabel = f'trans({perf_met})', ylabel = 'treatment')
            ax.set_title(s)
            
            pm_trmts = pm_tukey.groupsunique
            pm_pairinds = pm_tukey._multicomp.pairindices
    
            pm_trmt1 = pm_trmts[pm_pairinds[0]]
            pm_trmt2 = pm_trmts[pm_pairinds[1]]
            
            pm_results = pd.DataFrame(columns = tukey_results.columns)
            
            pm_results['trmt1'] = pm_trmt1
            pm_results['trmt2'] = pm_trmt2
...
grouping_dict = post_hoc_groups(results_df = tukey_results,factor_cols = ['trmt1','trmt2'],
                                block_cols = ['species','model'],
                                conclusion_col = 'reject',
                                response_col = 'perf_met')    
