...
outputs_df = pd.read_csv(os.path.join(output_dir,'streams-syn-aug_ML_results_compiled.csv'))

outputs_df['syn_aug'] = outputs_df.syn_aug.apply(lambda x: str(x))
  
#%% create some useful variables

abbrv = 'Streams-syn-aug'
...
syn_aug = outputs_df.syn_aug.unique()
syn_aug = np.sort(syn_aug)
...
#%% Do 3-way anova for both performance metrics.

rmse_anova = smf.ols('trans_val ~ C(model) + C(species) + C(syn_aug) +'+\
                          'C(model):C(syn_aug) + C(species):C(syn_aug) + C(model):C(species) +'+\
                              'C(model):C(species):C(syn_aug)',
                          data = test_rmses).fit()

sm.qqplot(rmse_anova.resid)

rsq_anova = smf.ols('trans_val ~ C(model) + C(species) + C(syn_aug) +'+\
                          'C(model):C(syn_aug) + C(species):C(syn_aug) + C(model):C(species) +'+\
                              'C(model):C(species):C(syn_aug)',
                          data = test_rsqs).fit()

sm.qqplot(rsq_anova.resid)

rmse_anova_rslt = sm.stats.anova_lm(rmse_anova,typ=2)

rsq_anova_rslt = sm.stats.anova_lm(rsq_anova,typ=2)
...
#%% perform pair-wise tukey test separated by species and model.
...
tukey_results = pd.DataFrame(columns = ['perf_met','species','model',
                                        'syn1','syn2','meandiff',
                                        'pvalue','reject'])
...
for perf_met in list(perf_mets.keys()):
    ...
    for s in species:
        ...
        for m in models:
            ...
            pm_sp = pm_df.loc[(pm_df.species==s)&(pm_df.model==m),:]
            ...
            pm_tukey.plot_simultaneous(ax = ax, xlabel = f'trans({perf_met})', ylabel = 'syn_aug')
            ax.set_title(s)
            
            pm_syns = pm_tukey.groupsunique
            pm_pairinds = pm_tukey._multicomp.pairindices
    
            pm_syn1 = pm_syns[pm_pairinds[0]]
            pm_syn2 = pm_syns[pm_pairinds[1]]
            
            pm_results = pd.DataFrame(columns = tukey_results.columns)
            
            pm_results['syn1'] = pm_syn1
            pm_results['syn2'] = pm_syn2
...
factor_cols = ['syn1','syn2']
block_cols = ['species','model']
conclusion_col = 'reject'
response_col = 'perf_met'

def post_hoc_groups(results_df,factor_cols,block_cols,conclusion_col,
                    response_col):
    
    results_df = results_df.copy()
    
    block_col = '_'.join(block_cols)
    
    for bc in block_cols:
        
        if bc==block_cols[0]:
            
            results_df[block_col] = results_df[bc]
            
        else:
            
            results_df[block_col] = results_df[block_col] +'_' + results_df[bc]
...
grouping_dict = post_hoc_groups(results_df = tukey_results,factor_cols = ['syn1','syn2'],
                                block_cols = ['species','model'],
                                conclusion_col = 'reject',
                                response_col = 'perf_met')    
...
tukey_results.to_csv(os.path.join(output_dir,'stats',
                                  f'{abbrv}_rsq-rmse_Tukey-by-spmod.csv'),
                     index=False)

factor_letters.to_csv(os.path.join(output_dir,'stats',
                                  f'{abbrv}_rsq-rmse_Tukey-by-spmod_factor-letters.csv'),
                     index=False)

letter_factors.to_csv(os.path.join(output_dir,'stats',
                                  f'{abbrv}_rsq-rmse_Tukey-by-spmod_letter-factors.csv'),
                     index=False)
