...
dl_comb_fn = 'streams-comb_DL_It0-19_results.csv'
dl_fil_fn = 'streams-fil_DL_It0-19_results.csv'
dl_unf_fn = 'streams-unf_DL_It0-19_results.csv'
...
results_files = [dl_comb_fn, dl_fil_fn, dl_unf_fn]
...
for f in results_files:
    ...
    if i == 0:
        ...
        model = f.split(sep='_')[0]
        
        model = model.split(sep='-')[1]
        
        results_df['filtration']=model
...
models = results_df.filtration.unique()
...
test_rmse_plot = sns.catplot(x='filtration',y='test rmse (ppm)',col = 'species',col_wrap=3,
...
test_rqrs_plot = sns.catplot(x='filtration',y='test r-squared',col = 'species',col_wrap=3,
...
for m in models_sub:
    ...
    for s in species_sub:
        ...
        true_vals = results_df.loc[(results_df.filtration == m)&
                                   (results_df.species == s)&
                                   (results_df.output=='y_true_test'),'value']
        
        pred_vals = results_df.loc[(results_df.filtration == m)&
                                   (results_df.species == s)&
...
#%% There are some highly negative r-squares skewing the plot

# test_rsqrs.loc[(test_rsqrs.filtration=='DL')&
...
test_rsqrs.loc[test_rsqrs['test r-squared']<-1,'test r-squared']=-1
...
fit_plot_sc_df = pd.DataFrame(columns = ['True','Predicted','filtration','Species'])
fit_plot_sc_df['True']=y_t_te_scaled.value
fit_plot_sc_df['Predicted']=y_h_te_scaled.value
fit_plot_sc_df['filtration']=y_h_te_scaled.filtration
fit_plot_sc_df['Species']=y_h_te_scaled.species

fit_plot_sc = sns.FacetGrid(fit_plot_sc_df,hue = 'filtration', col = 'Species',
                            col_wrap = 3,height = 4)
...
#sns.displot(data=errors_df, x="error", hue="filtration", col="Species", kind="kde",col_wrap=3)
sns.displot(data=errors_sub_df, x="error", hue="filtration", col="Species", kind="kde",
        ...
        for m in models:
            ...
            row = (rmses_norm_test.species == s) \
                & (rmses_norm_test.iteration == i) \
                    & (rmses_norm_test.filtration == m)
            
            rows = (y_true_tests.species == s) \
                & (y_true_tests.iteration == i) \
                    & (y_true_tests.filtration == m)
...
rmses_norm_test_av = pd.DataFrame(columns= ['filtration','species','value'])
...
for s in species:
    for m in models:
        ...
        rows = (rmses_norm_test.species == s) & (rmses_norm_test.filtration == m)
            
        rmse_norm_av = rmses_norm_test.loc[rows,'value'].mean()
        
        new_row = pd.DataFrame([[m,s,rmse_norm_av]],columns=['filtration','species','value'])
...
norm_test_rmse_plot = sns.catplot(x='filtration',y='normalized test rmse',
...
for s in species:
    pls_rows = (rmses_norm_test.species == s)&(rmses_norm_test.filtration == 'pls')
    dl_rows = (rmses_norm_test.species == s)&(rmses_norm_test.filtration == 'dl')
...
#sns.displot(data=errors_df, x="error", hue="filtration", col="Species", kind="kde",col_wrap=3)
sns.displot(data=rmses_norm_test, x="value", hue="filtration", col="species", kind="kde",
            col_wrap=3,linewidth=3,height=4,common_norm = False)
g = sns.displot(data=rmses_norm_test.loc[rmses_norm_test['species'].isin(species_sub),:],
            x="value", hue="filtration", col="species", kind="kde",
            col_wrap=3,linewidth=3,height=4,common_norm = False)
...
NSEs_test_plot = sns.catplot(x='filtration',y='NSE',
                                  col = 'species',col_wrap=3,
...
NSEs_test_av = pd.DataFrame(columns= ['filtration','species','value'])
...
for s in species:
    for m in models:
        ...
        rows = (NSEs_test.species == s) & (NSEs_test.filtration == m)
            
        NSE_av = NSEs_test.loc[rows,'value'].mean()
        
        new_row = pd.DataFrame([[m,s,NSE_av]],columns=['filtration','species','value'])
