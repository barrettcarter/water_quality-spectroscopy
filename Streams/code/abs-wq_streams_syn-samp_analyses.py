...
results_files = [pls_real_fn, pls_syn_fn]
...
for f in results_files:
    ...
    if i == 0:
        ...
        model = model.split(sep='-')[2]
        
        results_df['syn_samp']=model
...
models = results_df.syn_samp.unique()
...
test_rmse_plot = sns.catplot(x='syn_samp',y='test rmse (ppm)',col = 'species',col_wrap=3,
...
for s in species_sub:
    ...
    for m in models_sub:
        ...
        pred_vals = results_df.loc[(results_df.syn_samp == m)&
                                   (results_df.species == s)&
...
# test_rsqrs.loc[(test_rsqrs.syn_samp=='DL')&
...
fit_plot_sc_df = pd.DataFrame(columns = ['True','Predicted','syn_samp','Species'])
fit_plot_sc_df['True']=y_t_te_scaled.value
fit_plot_sc_df['Predicted']=y_h_te_scaled.value
fit_plot_sc_df['syn_samp']=y_h_te_scaled.syn_samp
fit_plot_sc_df['Species']=y_h_te_scaled.species

fit_plot_sc = sns.FacetGrid(fit_plot_sc_df,hue = 'syn_samp', col = 'Species',
                            col_wrap = 3,height = 4)
...
#sns.displot(data=errors_df, x="error", hue="syn_samp", col="Species", kind="kde",col_wrap=3)
sns.displot(data=errors_sub_df, x="error", hue="syn_samp", col="Species", kind="kde",
...
rmses_norm_test_av = pd.DataFrame(columns= ['syn_samp','species','value'])
...
for s in species:
    for m in models:
        ...
        rows = (rmses_norm_test.species == s) & (rmses_norm_test.syn_samp == m)
            
        rmse_norm_av = rmses_norm_test.loc[rows,'value'].mean()
        
        new_row = pd.DataFrame([[m,s,rmse_norm_av]],columns=['syn_samp','species','value'])
...
norm_test_rmse_plot = sns.catplot(x='syn_samp',y='normalized test rmse',
...
for s in species:
    pls_rows = (rmses_norm_test.species == s)&(rmses_norm_test.syn_samp == 'pls')
    dl_rows = (rmses_norm_test.species == s)&(rmses_norm_test.syn_samp == 'dl')
...
#sns.displot(data=errors_df, x="error", hue="syn_samp", col="Species", kind="kde",col_wrap=3)
sns.displot(data=rmses_norm_test, x="value", hue="syn_samp", col="species", kind="kde",
            col_wrap=3,linewidth=3,height=4,common_norm = False)
g = sns.displot(data=rmses_norm_test.loc[rmses_norm_test['species'].isin(species_sub),:],
            x="value", hue="syn_samp", col="species", kind="kde",
            col_wrap=3,linewidth=3,height=4,common_norm = False)
...
NSEs_test_plot = sns.catplot(x='syn_samp',y='NSE',
                                  col = 'species',col_wrap=3,
...
NSEs_test_av = pd.DataFrame(columns= ['syn_samp','species','value'])
...
for s in species:
    for m in models:
        ...
        rows = (NSEs_test.species == s) & (NSEs_test.syn_samp == m)
            
        NSE_av = NSEs_test.loc[rows,'value'].mean()
        
        new_row = pd.DataFrame([[m,s,NSE_av]],columns=['syn_samp','species','value'])
