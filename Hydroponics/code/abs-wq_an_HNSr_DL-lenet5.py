...
create_outputs(input_df = abs_wq_df, iterations = 20, autosave = True,
               output_path = os.path.join(output_dir,'HNS-NO3-N_syn-aug-False_DL_It0-19_results.csv'),
               subset_name = subset_name,syn_aug = False) # no synthetic samples

create_outputs(input_df = abs_wq_df, iterations = 20, autosave = True,
               output_path = os.path.join(output_dir,'HNS-NO3-N_syn-aug-True_DL_It0-19_results.csv'),
...
#                 +r'$test\/rmse =$'+str(np.round(test_rmse,3)), fontsize = 12)
#         # ticks = ax.get_yticks()
#         # print(ticks)
...
# make_plots(outputs_df,'Filtered and Unfiltered Stream Samples - DL')
