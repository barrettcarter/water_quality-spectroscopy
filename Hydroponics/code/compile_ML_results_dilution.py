...
This script if for compiling all ML modeling results for the HNS sample
dilution experiment (Ch. 3). Results are used by other scripts
...
# output_files = output_files[[8,9,11,12,13,17,20,21]] # select ML results files corresponding to experiment

# output_files.to_csv(os.path.join(output_dir,'stats','dilution_file_list.csv'))

#%% bring in data and concatenate

output_files = pd.read_csv(os.path.join(output_dir,'stats','dilution_file_list.csv'),
                           index_col = 0)['0']
...
outputs_df.to_csv(os.path.join(output_dir,'HNS-dilution_ML_results_compiled.csv'),index = False)
