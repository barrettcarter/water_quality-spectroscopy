...
test_rmses_path = os.path.join(hns_output_dir,'performance metrics', f'{abbrv}_test_rmse.csv')
test_rsqs_path = os.path.join(hns_output_dir,'performance metrics', f'{abbrv}_test_rsq.csv')
...
lev_trans = {'diluted':'diluted','undiluted':'undiluted'}
...
for pm in perf_mets:
    ...
    g = sns.catplot(data = pm_df, y = pm_lab, x = spcol, hue = 'treatment',
                    kind = 'box', palette = {'diluted':'lightsteelblue',
                                             'undiluted':'slategrey'},
    ...
    plt.savefig(os.path.join(hns_fig_dir,f'dilution_{pm}_boxplot.png'),dpi = 300)
