...
#         if (row == 0) and (col == 0):
#             axs[row,col].legend(loc = 'upper left',fontsize = 12)
...
#         ax.text(x_text,y_text,'$rmse_{tr} =$'+str(np.round(train_rmse,3))+'\n'
#                 +'$rmse_{te} =$'+str(np.round(test_rmse,3)), fontsize = 12)
#         # ticks = ax.get_yticks()
#         # print(ticks)
...
# make_plots(outputs_df,'HNSr - XGB-PCA')

#%% save output

# outputs_df.to_csv(output_dir+'HNSr_XGB_It0_results.csv',index=False)
